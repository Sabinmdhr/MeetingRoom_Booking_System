...
type InteractionMode = "none" | "drag" | "resize-top" | "resize-bottom" | "create";
...
export const TimeSlotSelector = ({
  ...
  const getYFromMinutes = (minutes: number) => (minutes - START_MINUTES) * MINUTE_HEIGHT;
  const getMinutesFromY = (y: number) => Math.round(y / MINUTE_HEIGHT) + START_MINUTES;
  ...
  const handleGridClick = (e: React.PointerEvent) => {
    ...
    const clickY = e.clientY - timelineRect.top + (timelineRef.current?.scrollTop || 0);
  ...
  const { formattedDate, changeDate, jumpToToday }= useRoomTimeslotViewModel();
